...
def user_input():
    ...
    if object_check['config_file'] != '':
        ...
        parser.add_argument('-m', '--metadata', default=True, help="Print the metadata of the image", action="store_true")
        parser.add_argument('-e', '--extract_metadata', default=True, help="Dumps all the logs into a text file", action="store_true")
        ...
        parser.add_argument('-su', '--save_unknown_format', help="tries to save images in unknown format", required=False)
    ...
    def download_image(self,image_url,image_format,main_directory,dir_name,count,print_urls,socket_timeout,prefix,print_size,no_numbering,no_download,save_source,img_src,silent_mode,thumbnail_only,format,ignore_urls,save_unknown):
        ...
        try:
            ...
            try:
                ...
                elif image_format == "" or not image_format or "." + image_format not in extensions:
                    if save_unknown:
                        image_name = image_name + ".jpg"
                    else:
                        download_status = 'fail'
